    ...
    def train(self):
        ...
        for epoch in range(1, end_epoch + 1):
            ...
            for step, (inputs, labels, indexes) in enumerate(self.data['train']):
                ...
                if self.cutmix and self.beta > 0:
                    if self.aug_sampler =='random':
                        self.aug_lam = np.random.beta(self.beta, self.beta)
                        rand_index = torch.randperm(inputs.size()[0]).cuda()
                        inputs_a = labels
                        inputs_b = labels[rand_index]
                        bbx1, bby1, bbx2, bby2 = rand_bbox(inputs.size(), self.aug_lam)
                        inputs[:, :, bbx1:bbx2, bby1:bby2] = inputs[rand_index, :, bbx1:bbx2, bby1:bby2]
                        self.aug_lam = 1 - ((bbx2 - bbx1) * (bby2 - bby1) / (input.size()[-1] * input.size()[-2]))
